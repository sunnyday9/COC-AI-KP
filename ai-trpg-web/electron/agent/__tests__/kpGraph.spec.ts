...
import { createKPGraph } from '../kpGraph.mjs'

describe('electron/agent/kpGraph', () => {
  it('routes combat intent and forces missing tool calls', async () => {
    const invokeLLM = async (msgs: Array<{ role: string; content: string }>) => {
  ...
  it('cleans text-simulated dice and still requests required tools', async () => {
    ...
    expect(Array.isArray(result.toolCalls)).toBe(true)
    expect(result.toolCalls.some((t: any) => t.name === 'skill_check')).toBe(true)
  })
})
